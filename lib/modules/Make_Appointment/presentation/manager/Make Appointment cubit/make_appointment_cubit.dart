...
class MakeAppointmentCubit
    extends Bloc<MakeAppointmentEvent, MakeAppointmentState> {
  ...
  static MakeAppointmentCubit get(BuildContext context) =>
    ...
    try {
      var result = await makeAppointmentRepo.makeAppointment(
        model: MakeAppointmentModel(
          ...
          note:
              noteController.text.isNotEmpty ? noteController.text : event.note,
        ),
      );

      if (result.error == null) {
        emit(state.copyWith(
            status: MakeAppointmentStatus.success,
            currentStep: 0,
            errorMessage: null));
      } else {
        emit(state.copyWith(
            status: MakeAppointmentStatus.error, errorMessage: result.error));
      }
    } catch (e) {
      emit(state.copyWith(
          status: MakeAppointmentStatus.error, errorMessage: e.toString()));
    }
  }

  Future<void> _onEditAppointment(
      EditAppointmentEvent event, Emitter<MakeAppointmentState> emit) async {
    emit(state.copyWith(status: MakeAppointmentStatus.loading));
    try {
      var result = await makeAppointmentRepo.editAppointment(
          model: event.model, id: event.model.id.toString());
      if (result.error == null) {
        emit(state.copyWith(
            status: MakeAppointmentStatus.success, errorMessage: null));
      } else {
        emit(state.copyWith(
            status: MakeAppointmentStatus.error, errorMessage: result.error));
      }
  ...
  Future<void> _onSetData(
      SetDataEvent event, Emitter<MakeAppointmentState> emit) async {
  ...
  Future<void> _onSetPatient(
      SetPatientEvent event, Emitter<MakeAppointmentState> emit) async {
  ...
  Future<void> _onChangeStep(
      ChangeStepEvent event, Emitter<MakeAppointmentState> emit) async {
    if (event.step >= 0 && event.step < 3) {
      // Assuming 3 steps as per original code
      emit(state.copyWith(currentStep: event.step));
    }
  }

  Future<void> _onPreviousStep(
      PreviousStepEvent event, Emitter<MakeAppointmentState> emit) async {
  ...
  Future<void> _onChangePage(
      ChangePageEvent event, Emitter<MakeAppointmentState> emit) async {
  ...
  Future<void> _onSetWidgetIndex(
      SetWidgetIndexEvent event, Emitter<MakeAppointmentState> emit) async {
    emit(state.copyWith(widgetIndex: event.index));
  }

  Future<void> _onValidateField(
      ValidateFieldEvent event, Emitter<MakeAppointmentState> emit) async {
